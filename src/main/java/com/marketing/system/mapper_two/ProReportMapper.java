...
public interface ProReportMapper {
    ...
    @Select("select squadId as squadd,count(*) as sum1 from project_task join project_info on project_task.proId=project_info.proId where  project_info.proState=4 and project_info.proType=1 and project_info.createrSquadId " +
    ...
    @Select("select squadId as squadd,count(*) as sum2 from project_task join project_info on project_task.proId=project_info.proId where  project_info.proState=4 and project_info.proType=2 and project_info.createrSquadId " +
            " like '%${creatersquadid}%' and project_info.creater like '%${creater}%' and project_info.createDate >= #{createdate1} and project_info.createDate <= #{createdate2} and project_info.finishDate >=#{finishdate1} and project_info.finishDate <= #{finishdate2} and " +
            " project_info.onlineDate >= #{onlinedate1} and project_info.onlineDate <=#{onlinedate2}  and project_info.proName like '%${param}%' GROUP BY project_task.squadId ")
